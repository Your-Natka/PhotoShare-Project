...
збереться Docker image
запустить release command (якщо є alembic — виконає міграції)
створить / оновить VM
запустить додаток

Якщо зʼявляться помилки - виправити.

Подивитися лог запуску в реальному часі
fly logs
Там буде видно:

чи запустився Uvicorn
чи є помилки з Cloudinary
чи база підключена
чи Redis доступний
